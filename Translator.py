...
class Translator_App(object):
    def __init__(self,master):
        ...
        def translate_page(self):
            ...
            self.word_entry.grid(column=0,row=1,columspan=3,padx=5,pady=5)
            ...
            self.word_frame=LabelFrame(self.translate_tab,text="word:",wedth=300,height=50)
            self.word_frame.grid(column=0,row=4,columspan=3)
            self.word_frame.grid_propagate(0)

            self.word_result =Label(self.word_frame,text="")
            self.word_result.grid_propagate(0)

            self.noun_frame= LabelFrame(self.translate_tab,text="Noun:",widt=300,height=100)
            self.noun_frame.grid(column=0,row=5,columspan=3)
            self.noun_frame.grid_propagate(0)
            self.noun_result =Label(self.noun_frame,text="")
            self.noun_result.grid_propagate(0)

            self.verb_frame= LabelFrame(self.translate_tab,text="Verb:",widt=300,height=100)
            self.verb_frame.grid(column=0,row=6,columspan=3)
            self.verb_frame.grid_propagate(0)
            self.verb_result =Label(self.verb_frame,text="")
            self.verb_result.grid_propagate(0)


            def traslation_function(self):
            ...
            def definition(self,word):
                ...
                try:
                    ...
                    nouns.append("No Date")
                    verbs.append("No Date")
                return nouns,verbs

            def translate_func(self,words,language):
                translator = Translator(service_urls=["translate.google.com"])
                traslation =translator.translate(words,dest=language)
                return traslation
